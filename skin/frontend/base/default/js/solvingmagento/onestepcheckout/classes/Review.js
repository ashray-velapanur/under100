...
Review.prototype = {
    ...
    reviewUpdated: function (transport) {
        ...
        if (!response.error && this.readyToSave) {
            if ($('order_submit_button')) {
                if ($('p_method_stripe') && $('p_method_stripe').checked) {
                    this.stripeSave();
                }
                $('order_submit_button').addClassName("checkout-finish-btn");
                $('order_submit_button').down().down().update(checkout.buttonSaveText);
            }
        } else {
            this.readyToSave = false;
            if ($('order_submit_button')) {
                $('order_submit_button').removeClassName('checkout-finish-btn');
    ...
    stripeSave: function() {
        var methodCode = TemplateTag.Stripe.MethodCode;
        try {
            Stripe.createToken({
                address_line1: $('billing:street1').value,
                address_zip: $('billing:postcode').value,
                name: $(methodCode + '_cc_owner').value,
                number: $(methodCode + '_cc_number').value,
                cvc: $(methodCode + '_cc_cvc').value,
                exp_month: $(methodCode + '_cc_expiration_month').value,
                exp_year: $(methodCode + '_cc_expiration_year').value

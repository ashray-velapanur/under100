...
class Mage_Review_Model_Review extends Mage_Core_Model_Abstract
{
    ...
    {
        try {
            if ($this->getStatusId() != $this->getOrigData('status_id') && $this->getStatusId() == '1') {
                $notifyId = Mage::getModel('catalog/product')->load($this->getEntityPkValue())->getProductOwnerId();
                if ($notifyId) {
                    Mage::getModel('notifications/notification')->setType('post_reviewed')->setNotifyId($notifyId)->setDataId($this->getId())->save();
                }
            }
        } catch (Exception $e) {
        }

...
class Solvingmagento_OneStepCheckout_Block_Onestep_Shipping extends Mage_Checkout_Block_Onepage_Shipping
{
    ...
    public function getDefaultAddress()
    {
        ...
        if (empty($addressId)) {
            $address = $this->getCustomer()->getPrimaryShippingAddress() ?: $this->getCustomer()
    ...
    public function getAddressesHtmlSelect($type)
    {
        if ($this->isCustomerLoggedIn()) {
            ...
            foreach ($this->getCustomer()->getAddresses() as $address) {
                $options[] = array(
                    'value' => $address->getId(),
                    'label' => $this->escapeHtml($address->getAddressName()?:$address->format('html'))
                );
            }
            ...
            foreach ($options as $option) {
                ...
                $html .= '/><label for="' . $type . '-address-id-' . $option['value'] . '">' . $option['label']
                    . '</label>';
            ...
            $html .= '<div><input type="radio" name="' . $type . '_address_id" value="" id="' . $type
                . '-address-id-">';
            $html .= '<label for="' . $type . '-address-id-">' . Mage::helper('checkout')->__('New address')
                . '</label>';
            ...
            $select = $this->getLayout()->createBlock('core/html_select')->setName($type . '_address_id')->setId($type
                . '-address-select')->setClass('address-select')->setExtraParams('onchange="' . $type
                . '.newAddress(!this.value)"')->setValue($addressId)->setOptions($options);

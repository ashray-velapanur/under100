...
class GaussDev_CustomerImages_Helper_Image extends Mage_Core_Helper_Abstract
{
    public $width = null;
    public $height = null;
    public $rawImg = '';
    ...
    /**
     * @param      $customer
     * @param bool $bigPlaceholder
     *
     * @return $this
     */
    public function init($customer, $bigPlaceholder = false)
    {
        $this->_reset();
        $img = null;

        if (empty($this->placeHolder)) {
            $placeholderSize = $bigPlaceholder ? '228' : '62';
            $this->placeHolder = Mage::getDesign()->getSkinUrl("images/catalog/product/placeholder/avatar-placeholder-{$placeholderSize}.png");
        }
        ...
        if ($customer) {
            $img = Mage::getBaseUrl('media') . 'customer' . $customer->getProfileImage();
        }
    ...
    public function adaptiveResize()
    {
        ...
        $processor->crop(floor($diffHeight * $this->_topRate),
                         floor($diffWidth / 2),
                         ceil($diffWidth / 2),
    ...
    {
        $cache = BP . DS . 'media' . DS . 'customer' . DS . 'cache' . DS;
